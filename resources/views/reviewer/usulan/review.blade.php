...
<div class="max-w-7xl mx-auto mt-8 px-4 sm:px-6 lg:px-8">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4 gap-4">
        ...
        <a href="{{ route('reviewer.dashboard') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md text-sm font-semibold text-gray-700 hover:bg-gray-50 shadow-sm">
            &larr; Kembali
        </a>
    </div>
        ...
        <div class="file-viewer-wrapper">
                ...
                <div class="bg-white shadow sm:rounded-lg border border-gray-200 mb-6" style="min-height: 800px;">
                    ...
                    <div class="px-4 py-3 border-t bg-gray-50 text-right">
                        <a href="{{ route('reviewer.usulan.download_file', [$usulan->id, $filename]) }}"
                            class="px-3 py-2 border border-blue-300 rounded-md text-blue-700 text-sm hover:bg-blue-50 shadow-sm" download>
                            Unduh File Asli
                        </a>
                    </div>
                </div>
            @elseif($filename)
                <div class="p-4 bg-yellow-50 border border-yellow-200 text-yellow-600 text-sm text-center rounded mb-6">
                    File ditemukan ({{ $filename }}), tetapi hanya PDF yang bisa dipreview. Silakan 
                    <a href="{{ route('reviewer.usulan.download_file', [$usulan->id, $filename]) }}" class="font-semibold underline">
                        Unduh File
                    </a>.
                </div>
            @else
        ...
        {{-- RIGHT: Review Form --}}
            ...
            @else
                <div class="bg-white shadow sm:rounded-lg border border-gray-200 p-6 text-center">
                    ...
                    <p class="text-sm text-gray-500 mt-2">Anda dapat kembali ke 
