    ...
    <div class="flex-1 flex flex-col ml-64">
        ...
        <main class="flex-1 p-6">
            ...
            <div class="bg-white shadow rounded-lg p-6">
                <h2 class="text-2xl font-bold text-gray-800 mb-4">Edit Pengumuman</h2>
            ...
            <form action="{{ route('admin.pengumuman.update', $pengumuman->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="bg-white shadow rounded-lg p-6 mt-6">
                    <div class="mb-4">
                        ...
                        <input type="text" id="judul" name="judul" value="{{ $pengumuman->judul }}" class="w-full border border-gray-300 p-2 rounded" required>
                    </div>
                    <div class="mb-4">
                        <label for="isi" class="block text-gray-700 font-bold mb-2">Isi:</label>
                        <textarea id="isi" name="isi" rows="5" class="w-full border border-gray-300 p-2 rounded" required>{{ $pengumuman->isi }}</textarea>
                    </div>
                    <div class="mb-4">
                        <label for="kategori" class="block text-gray-700 font-bold mb-2">Kategori:</label>
                        <input type="text" id="kategori" name="kategori" value="{{ $pengumuman->kategori }}" class="w-full border border-gray-300 p-2 rounded" required>
                    </div>
                    <div class="mb-4">
                        <label for="gambar" class="block text-gray-700 font-bold mb-2">Gambar:</label>
                        @if($pengumuman->gambar)
                            <img src="{{ asset('storage/' . $pengumuman->gambar) }}" alt="{{ $pengumuman->judul }}" class="w-32 h-32 object-cover mb-2 rounded">
                        @endif
