<aside class="w-64 bg-white shadow-lg flex flex-col h-screen fixed top-0 overflow-y-auto" 
    x-data="{ 
        // MEMPERBARUI LOGIKA ACTIVE STATE: Mencakup usulan, laporan kemajuan, dan laporan akhir
        openPenelitian: {{ request()->routeIs('dosen.usulan.*') || request()->routeIs('dosen.laporan_kemajuan.*') || request()->routeIs('dosen.laporan_akhir.*') || request()->routeIs('dosen.penelitian.*') || request()->routeIs('dosen.pengabdian.*') ? 'true' : 'false' }}, 
        openPengumuman: {{ request()->routeIs('dosen.pengumuman.*') ? 'true' : 'false' }} 
    }">
    <div class="p-6 border-b border-gray-200 bg-white">
        <h1 class="text-xl font-extrabold text-gray-900 tracking-wider">P3M Dosen</h1>
    </div>
    ...
    <nav class="flex-1 mt-4 space-y-1 px-3">
        ...
        <a href="{{ route('dosen.dashboard') }}" 
            class="block py-3 px-4 rounded-lg flex items-center gap-3 transition duration-150 ease-in-out 
                    {{ request()->routeIs('dosen.dashboard') ? $activeClass : $defaultClass }}">
        ...
        <a href="{{ route('dosen.pengumuman.index') }}" 
            class="block py-3 px-4 rounded-lg flex items-center gap-3 transition duration-150 ease-in-out 
                    {{ request()->routeIs('dosen.pengumuman.*') ? $activeClass : $defaultClass }}">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-2.236 9.168-5.584C18.354 1.832 18 3.65 18 4.5c0 1.25.333 2.45 1 3.5" />
            </svg>
            Pengumuman Program
        </a>
        ...
        <button @click="openPenelitian = !openPenelitian" 
                class="w-full flex items-center justify-between py-3 px-4 rounded-lg transition duration-150 ease-in-out 
                        {{ request()->routeIs('dosen.usulan.*') || request()->routeIs('dosen.laporan_kemajuan.*') || request()->routeIs('dosen.laporan_akhir.*') || request()->routeIs('dosen.penelitian.*') || request()->routeIs('dosen.pengabdian.*') ? $activeClass : $defaultClass }}">
        ...
        <div x-show="openPenelitian" x-collapse x-cloak class="pl-8 space-y-1">
            ...
            <a href="{{ route('dosen.usulan.index') }}" 
               class="block py-2 px-4 rounded-lg transition duration-150 ease-in-out 
                       {{ request()->routeIs('dosen.usulan.*') ? $activeSubClass : $defaultSubClass }}">
                Daftar Usulan
            </a>
            ...
            <a href="{{ route('dosen.laporan-kemajuan.index') }}" 
               class="block py-2 px-4 rounded-lg flex items-center gap-3 transition duration-150 ease-in-out 
                       {{ request()->routeIs('dosen.laporan_kemajuan.*') ? $activeSubClass : $defaultSubClass }}">
                Laporan Kemajuan
            </a>
            
            {{-- MENU BARU: LAPORAN AKHIR --}}
            <a href="{{ route('dosen.laporan_akhir.index') }}" 
               class="block py-2 px-4 rounded-lg flex items-center gap-3 transition duration-150 ease-in-out 
                       {{ request()->routeIs('dosen.laporan_akhir.*') ? $activeSubClass : $defaultSubClass }}">
                Laporan Akhir
            </a>
            
            <a href="#" 
               class="block py-2 px-4 rounded-lg transition duration-150 ease-in-out 
                       {{ request()->routeIs('dosen.pengabdian.*') ? $activeSubClass : $defaultSubClass }}">
                Daftar Pengabdian
            </a>
        </div>
