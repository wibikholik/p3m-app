<!--
    File ini berisi perbaikan untuk sidebar Dosen, diubah menjadi tema TERANG (LIGHT THEME)
    dengan logika active state dinamis menggunakan rute Laravel Blade.
-->
<aside class="w-64 bg-white shadow-lg flex flex-col h-screen fixed top-0 overflow-y-auto" 
       x-data="{ 
            openPenelitian: {{ request()->routeIs('dosen.usulan.*') || request()->routeIs('dosen.penelitian.*') || request()->routeIs('dosen.pengabdian.*') ? 'true' : 'false' }}, 
    ...
    <nav class="flex-1 mt-4 space-y-1 px-3">
        ...
        <button @click="openPenelitian = !openPenelitian" 
                class="w-full flex items-center justify-between py-3 px-4 rounded-lg transition duration-150 ease-in-out 
                       {{ request()->routeIs('dosen.usulan.*') || request()->routeIs('dosen.penelitian.*') || request()->routeIs('dosen.pengabdian.*') ? $activeClass : $defaultClass }}">
