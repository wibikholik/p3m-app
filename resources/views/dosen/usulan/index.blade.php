                        ...
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                @php
                                    ...
                                    $displayStatus = ucfirst($usulan->status);
                                    $class = 'bg-gray-100 text-gray-800 border-gray-200';

                                    $isRevisionFileEmpty = empty($usulan->file_revisi);

                                    if ($currentStatus == 'diajukan') {
                                        $class = 'bg-yellow-100 text-yellow-800 border-yellow-200';
                                        $displayStatus = 'Menunggu Penilaian';
                                    } elseif ($currentStatus == 'diterima') {
                                        $class = 'bg-green-100 text-green-800 border-green-200';
                                        $displayStatus = 'Diterima';
                                        // Tambahkan info siap lanjut laporan
                                        if(isset($usulan->status_lanjut) && $usulan->status_lanjut == 'siap_laporan') {
                                            $displayStatus .= ' 📝 Siap Laporan';
                                        }
                                    } elseif ($currentStatus == 'ditolak') {
                                        $class = 'bg-red-100 text-red-800 border-red-200';
                                    } elseif (in_array($currentStatus, ['revisi', 'menunggu revisi'])) {
                                        if ($isRevisionFileEmpty) {
                                            $class = 'bg-orange-100 text-orange-800 border-orange-200';
                                            $displayStatus = 'Menunggu Unggahan Revisi';
                                        } else {
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end gap-3">
                                    ...
                                    {{-- Tombol Buat Laporan Kemajuan --}}
                                    @if(isset($usulan->status_lanjut) && $usulan->status_lanjut == 'siap_laporan')
                                        <a href="{{ route('dosen.laporan-kemajuan.create', $usulan->id) }}" class="text-green-600 hover:text-green-900 bg-green-50 px-3 py-1 rounded hover:bg-green-100 transition">
                                            Buat Laporan
                                        </a>
                                    @endif

                                    {{-- Tombol Edit / Unggah Revisi --}}
                                    @if($currentStatus == 'diajukan')
                                    ...
                                    @elseif(in_array($currentStatus, ['revisi', 'menunggu revisi']) && $isRevisionFileEmpty)
                                        <a href="{{ route('dosen.usulan.edit', $usulan->id) }}?action=upload" class="text-green-600 hover:text-green-900 bg-green-50 px-3 py-1 rounded hover:bg-green-100 transition">
                                            Unggah Revisi
                                        </a>
                                    @endif
                                </div>
